    ...
    <SelectContext.Provider value={{ open, setOpen, value, onChange }}>
      <div ref={selectRef} className="relative inline-block w-fit">
        <button
          ...
          className={`w-fit focus:ring-primary focus:ring-2 focus:border-transparent disabled:opacity-50 disabled:cursor-not-allowed  border-gray-300  border rounded-md  px-3 py-2 flex items-center justify-start gap-2 min-h-[30px] ${className}`}
        >
          {selectedLabel || <span className="text-gray-400">{placeholder}</span>}
        </button>

        {open && (
          <div className="absolute mt-1.5 w-fit bg-primary-foreground overflow-hidden rounded-md shadow-sm z-50 ">
            {children}
          </div>
        )}
      </div>
